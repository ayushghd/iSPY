...
public class TrackerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTransportIdEditText.setText(transportID.replace(",","."));
    ...
    private void checkInputFields() {
        ...
        final String part= parts[0]; // 004
        if ((!mTransportIdEditText.getText().toString().equals(part)) || mTransportIdEditText.length() == 0 || mEmailEditText.length() == 0 ||
                mPasswordEditText.length() == 0) {
            Toast.makeText(TrackerActivity.this, R.string.missing_inputs, Toast.LENGTH_SHORT).show();
        } else {
            // Store values.
            SharedPreferences.Editor editor = mPrefs.edit();
            editor.putString(getString(R.string.transport_id), mTransportIdEditText.getText().toString().replace('.',','));
